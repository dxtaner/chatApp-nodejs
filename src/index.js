...
io.on("connection", (socket) => {
  ...
  socket.on("sendPrivateMessage", ({ to, message }, callback) => {
    const sender = getUser(socket.id);
    const receiver = getUsersInRoom(sender.room).find(
      (user) => user.username === to
    );

    if (!receiver) {
      return callback(`User ${to} not found`);
    }

    io.to(receiver.id).emit(
      "message",
      generateMessage(sender.username, message)
    );
    callback();
  });
